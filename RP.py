...
def read_papercsv(filePaperPath:str,fileCourtPath:str):
    ...
    with open(fileCourtPath, 'rt') as f:
        reader = csv.DictReader(f)
        for row in reader:
            kwargs={'Form':row['Form'],"all":row["all"],"title":row["title"]}
...
def endPaper(paper_dict:dict):
    ...
    """
    od = OrderedDict(paper_dict)
    
    papers=[]
    courts=[]

    od=sorted(od.items(), key=lambda x: x[1]['Form'], reverse=True)
    for i in od:
        print(i)
        """
        if i["Form"] in ["単行本","雑誌","論文集","判批"]:
            make_PaperRef(i["id"])
            papers.append(i)
        if i["Form"] in ["判例"]:
            courts.append((i["all"],i["court"],int(i["year"]),i["Form"]))
        """
